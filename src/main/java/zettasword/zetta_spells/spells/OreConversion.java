...
public class OreConversion extends RaySpell {
    ...
    protected boolean onBlockHit(CastContext ctx, BlockHitResult blockHit, Vec3 origin) {
        ...
        if (ctx.world().isEmptyBlock(blockPos)) {
            if (!ctx.world().isClientSide && BlockUtil.canPlaceBlock(ctx.caster(), ctx.world(), blockPos))
                ctx.world().setBlockAndUpdate(blockPos, ZSBlocks.ORE_CONVERSION_BLOCK.get().defaultBlockState());
            return true;
        }
        return false;
    }
    ...
    protected void spawnParticle(CastContext ctx, double x, double y, double z, double vx, double vy, double vz) {
        ParticleBuilder.create(EBParticles.SPARKLE).color(0xFF9800).pos(x, y, z).collide(true).spawn(ctx.world());
    }
