...
import zettasword.zetta_spells.system.AreaLifter;
...
public class GroundAscend extends RaySpell {
    ...
    protected boolean onEntityHit(CastContext ctx, EntityHitResult entityHit, Vec3 origin) {
        if (!ctx.world().isClientSide){
            ...
            return AreaLifter.liftArea(level, entityHit.getEntity().getOnPos(), 16);
        }
        return true;
    }
    ...
    protected boolean onBlockHit(CastContext ctx, BlockHitResult blockHit, Vec3 origin) {
        if (!ctx.world().isClientSide){
            ServerLevel level = (ServerLevel) ctx.world();
            return AreaLifter.liftArea(level, blockHit.getBlockPos(), 16);
        }
        return true;
    }
