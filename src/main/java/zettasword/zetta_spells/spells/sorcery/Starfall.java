...
import zettasword.zetta_spells.entity.construct.StarfallSigil;
import zettasword.zetta_spells.entity.construct.sigils.StarFloor;

public class Starfall extends RaySpell {
    public Starfall(){}
    ...
    protected boolean onEntityHit(CastContext ctx, EntityHitResult entityHit, Vec3 origin) {
        if (!ctx.world().isNight()) return false;
        int diff = 0;
        while(ctx.world().getBlockState(entityHit.getEntity().blockPosition().relative(Direction.UP, diff)).isAir() && diff < 15){
            diff++;
        }
        if (diff > 0 && !ctx.world().isClientSide()){
            StarfallSigil starfallSigil = new StarfallSigil(ctx.world());
            starfallSigil.setPos(entityHit.getEntity().blockPosition().relative(Direction.UP, diff).getCenter().add(0, -0.6, 0));
            starfallSigil.setLifetime(400);
    ...
    protected boolean onBlockHit(CastContext ctx, BlockHitResult blockHit, Vec3 origin) {
        if (!ctx.world().isNight()) return false;
        int diff = 0;
        while(ctx.world().getBlockState(blockHit.getBlockPos().relative(Direction.UP, diff+1)).isAir() && diff < 15){
            diff++;
        }
        if (diff > 0 && !ctx.world().isClientSide()){
            StarfallSigil starfallSigil = new StarfallSigil(ctx.world());
            starfallSigil.setPos(blockHit.getBlockPos().relative(Direction.UP, diff).getCenter().add(0, -0.6, 0));
            starfallSigil.setLifetime(400);
            starfallSigil.setSizeMultiplier(8F);
            starfallSigil.setCaster(ctx.caster());
            ctx.world().addFreshEntity(starfallSigil);
            ...
            floor.setPos(blockHit.getBlockPos().getCenter().add(0, +0.6, 0));
    ...
    protected @NotNull SpellProperties properties() {
        return SpellProperties.builder()
                .assignBaseProperties(SpellTiers.MASTER, Elements.SORCERY, SpellTypes.ATTACK, SpellAction.POINT_UP, 400, 50, 70)
